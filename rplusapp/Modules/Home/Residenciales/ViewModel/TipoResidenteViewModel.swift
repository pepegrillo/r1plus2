...
class TipoResidenteViewModel {
    ...
    var listArray : [DataTipoResidente] = []{
    ...
    func requestGetTipoResidente(paramIdResidencial: String) {
        print("inicio")
        TipoResidenteWS().getTipoResidenteList(idResidencial: paramIdResidencial, completion: { (dataResponse, error) in
            ...
            //setting model
            print("respuesta WS ----> \(dataResponse ?? .init())")
            self.listArray.append(contentsOf: (dataResponse?.data ?? .init()))
