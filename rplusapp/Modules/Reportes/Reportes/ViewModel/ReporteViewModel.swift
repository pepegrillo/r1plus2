...
class ReporteViewModel {
    ...
    //Array of Winner Model class
    var listArrayMisReportes : [DataReporte] = []{
        //Reload data when data set
        didSet{
            reloadList()
        }
    ...
    func requestGetReporte() {
        print("inicio")
        AlertaWS().getAlertaList(completion: { (dataResponse, error) in
    ...
    // function mis reportes por usuario
    func requestGetMisReporte() {
        print("inicio")
        ReporteWS().getReporteList(completion: { (dataResponse, error) in
